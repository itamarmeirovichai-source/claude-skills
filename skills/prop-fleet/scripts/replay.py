...
def trail_summary(s: pd.DataFrame, bars: dict, stop_col: str,
    ...
    print(f"\n  רגישות לטריגר — הסולם כולו מוזז, לא רק מדרגה אחת:")
    print(f"{'טריגר':>8} {'גררו':>7} {'תוחלת':>9} {'גבול תחתון':>12}")
    for trig in TRAIL_GRID:
        # מזיזים את הסולם כולו, לא את המדרגה הראשונה בלבד.
        #
        # הגרסה הקודמת החזיקה את המדרגה השנייה קבועה על 1.5R, ואז כל
        # טריגר שנסרק מעל 1.5 נבלע בה: 1.50 ו-2.00 החזירו שורות
        # זהות בדיוק, כי בשני המקרים מה שפעל היה אותה מדרגה שנייה.
        # טבלה שמדפיסה את אותו תרחיש פעמיים תחת שתי כותרות אינה
        # סריקת רגישות — היא נראית כמוה.
        #
        # ההזזה שומרת על צורת הסולם של הבוט: נקודת איזון בטריגר,
        # ונעילת חצי חצי-סיכון מעליו. ב-trig=1.0 זה מחזיר בדיוק את
        # TRAIL_LADDER, וזו הבדיקה שהשורה הזאת לא סטתה.
        gap = TRAIL_LADDER[1][0] - TRAIL_LADDER[0][0]
        g = replay(s, bars, stop_col, tp_col,
                   trail_ladder=((trig, TRAIL_LADDER[0][1]),
                                 (trig + gap, TRAIL_LADDER[1][1])))
        gf = g[g.status == "filled"]
        if len(gf) < MIN_TRADES:
            continue
