...
- `scripts/test_paper_check.py` — 17 tests, each building a row that looks entirely normal to the eye and fails only on the ratio, plus the rule that a row which cannot be compared counts as unchecked rather than as a pass. Seven of them exist because the first version of this checker mixed scales itself: with `exec_entry` empty it fell back to the signal level, compared a futures level against an ETF exit price, and flagged every profitable long and every losing short as a contradiction. It reported three real trades as corrupt that were merely unverifiable.
