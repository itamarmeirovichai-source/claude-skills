...
- `scripts/test_paper_check.py` — 10 tests, each building a row that looks entirely normal to the eye and fails only on the ratio, plus the rule that a row which cannot be compared counts as unchecked rather than as a pass.
