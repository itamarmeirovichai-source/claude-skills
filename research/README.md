...
| `run4-blender-handoff/` | Phase 4 — build the Blender scene | Partial: a contract-correct scene was built headless with `bpy` here (`out/`), but the scans and the final render need the Mac |
...
- **Run 4 ran only partially.** No Blender MCP and no scans here, so the scene was built headless with the `bpy` module from procedural stand-in geometry. The rig is correct — names, origins, all 8 beats keyed, the continuous camera take, the label anchor track, the glTF pair with all 15 contract names verified inside the files. It does **not** look like real food yet, and `PART-F-build-log.md` says so plainly.
