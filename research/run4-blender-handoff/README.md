...
`RUN4-PROMPT.md` is the complete, ready-to-paste input for it: the Run 4 instructions and steps, plus exactly the four things the master prompt says to carry over from Run 3 — the 8-beat sequence, the asset contract, the motion tokens, and the 3D part of the asset plan. It was assembled programmatically from the Run 3 files, not retyped, so the spec and the scene cannot drift apart.
