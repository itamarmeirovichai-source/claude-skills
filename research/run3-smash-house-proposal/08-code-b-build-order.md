...
Plays the same 8 beats with primitives and clipping-plane caps. Every value is a pure function of `p`, so scrubbing backwards is exact. Ship this until the Blender frames exist; swapping is a one-flag change.

```js
import * as THREE from 'three'
import { gsap, ScrollTrigger, reduced, isMobile, dprCap } from '../lib/scroll.js'
...
// deterministic pseudo-noise so the shake is identical when scrubbed backwards
const hashNoise = (x) => Math.sin(x * 127.1) * 43758.5453 % 1
...
export function initSequenceFallback(root) {
  ...
  const fill = new THREE.DirectionalLight(0xE51144, 0.5); fill.position.set(3, 0.5, 3); scene.add(fill)
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const layers = LAYERS.map((L) => {
      const geo = new THREE.CylinderGeometry(L.r, L.r * 0.97, L.h, 48)
      const mat = new THREE.MeshStandardMaterial({ color: L.color, roughness: 0.55, metalness: 0 })
    ...
    g.userData = {
      ...
      driftX: (i - 2) * 0.55,
      driftZ: (i % 2 ? 1 : -1) * 0.3,
      spinAxis: new THREE.Vector3(hashNoise(i + 1), 1, hashNoise(i + 7)).normalize(),
      w0: 5.5 + i * 0.8,
    }
    scene.add(g)
    burgers.push(g)
  ...
  const stackHeight = LAYERS.reduce((s, L) => s + L.h, 0)
  const centerIndex = (LAYERS.length - 1) / 2

  // --- cut: two clipping planes + explicit cap planes ---
  const planeL = new THREE.Plane(new THREE.Vector3(1, 0, 0), 0)   // keeps x < c
  const planeR = new THREE.Plane(new THREE.Vector3(-1, 0, 0), 0)  // keeps x > c
  const capMat = new THREE.MeshStandardMaterial({ color: INTERIOR, roughness: 0.45, side: THREE.DoubleSide })
  const capsL = new THREE.Group(); capsL.name = 'cut_face_left'
  const capsR = new THREE.Group(); capsR.name = 'cut_face_right'
  LAYERS.forEach((L) => {
    const geo = new THREE.PlaneGeometry(L.r * 2, L.h)
    ;[[capsL, -Math.PI / 2], [capsR, Math.PI / 2]].forEach(([grp, ry]) => {
  ...
  const target = new THREE.Vector3(0, stackHeight / 2, 0)

  function update(p) {
    // ---- beat 1: ballistic toss ----
    const t1 = local(p, BEATS.toss)
    burgers.forEach((g, i) => {
      const d = g.userData
      const tt = Math.max(0, (t1 - d.phase) / (1 - d.phase)) * 1.25
      g.position.set(
        d.driftX * tt,
        -4 + d.v0 * tt - 0.5 * 9.8 * tt * tt * 0.35,
        d.driftZ * tt)
      const angle = (d.w0 / 2.4) * (1 - Math.exp(-2.4 * (t1 + local(p, BEATS.closeup))))
      g.quaternion.setFromAxisAngle(d.spinAxis, angle)
      if (i !== 2) {
        const fall = local(p, BEATS.closeup)
        g.position.y -= fall * 8
        g.visible = fall < 0.95
      }
    })

    // ---- beat 2: bullet time — objects freeze, camera keeps orbiting ----
    const t2 = local(p, BEATS.bullet)
    if (p >= BEATS.bullet[0] && p < BEATS.closeup[0]) {
      burgers.forEach((g) => { g.position.y = g.position.y }) // held: position is already a function of t1, which is pinned at 1
    }

    // ---- beat 3: hero settles to face camera ----
    const t3 = local(p, BEATS.closeup)
    hero.position.lerpVectors(hero.position, new THREE.Vector3(0, 0, 0), t3)
    if (t3 > 0.85) hero.quaternion.slerp(faceCam, (t3 - 0.85) / 0.15)

    // ---- beat 4+6: explode, then slam back ----
    const e = easeBack(local(p, BEATS.explode))
    const smashT = local(p, BEATS.smash)
    const antic = THREE.MathUtils.clamp(smashT / 0.3, 0, 1)          // 59-62%: pull further out
    const impact = easeExpoIn(THREE.MathUtils.clamp((smashT - 0.3) / 0.2, 0, 1)) // 62-64%
    hero.userData.layers.forEach((m, i) => {
      const stagger = THREE.MathUtils.clamp((local(p, BEATS.explode) - i * 0.05) / (1 - i * 0.05), 0, 1)
      const spread = easeBack(stagger) * (1 + antic * 0.08) * (1 - impact)
      m.position.y = m.userData.restY + (i - centerIndex) * 0.85 * spread
    ...
    // ---- beat 6: impact squash + camera shake ----
    const recoil = THREE.MathUtils.clamp((smashT - 0.5) / 0.5, 0, 1)
    const squash = impact * (1 - recoil)
    ...
    const showCut = cutting > 0.02
    planeL.constant = 0 + open * 0.0
    planeR.constant = 0 - open * 0.0
    hero.userData.layers.forEach((m) => {
      m.material.clippingPlanes = showCut ? [planeL] : null
      m.material.clipShadows = true
      // squash under the blade as it passes
      const under = THREE.MathUtils.clamp(1 - Math.abs(cutting - 0.5) * 4, 0, 1)
      m.scale.y = 1 - 0.06 * under
    })
    capsL.children.forEach((c) => { c.visible = showCut })
    capsR.children.forEach((c) => { c.visible = showCut })
    capsL.position.x = -open * 0.22
    capsR.position.x = open * 0.22
    capsL.rotation.y = -open * 0.18
    capsR.rotation.y = open * 0.18
    ...
    const height = 0.6 + local(p, BEATS.toss) * 1.4 + pull * 0.7
    const shake = impact * (1 - recoil)
    camera.position.set(
      Math.sin(orbit) * radius + hashNoise(p * 900) * 0.06 * shake,
      height + hashNoise(p * 770) * 0.05 * shake,
      Math.cos(orbit) * radius)
    camera.lookAt(target)
    camera.rotation.z += hashNoise(p * 610) * 0.012 * shake
    camera.getWorldQuaternion(tmpQ); faceCam.copy(tmpQ)
  }
...
export async function loadHero(renderer, url = '/models/hero.glb', names = HERO_LAYERS) {
  ...
  const map = new Map()
  root.traverse((o) => { if (o.isMesh) map.set(o.name, o) })
  ...
  // origins are authored at each layer's own centre; stack axis is +Y after the Blender +Z conversion
  names.forEach((n) => { map.get(n).userData.restY = map.get(n).position.y })
  draco.dispose(); ktx2.dispose()
  return { root, layers: names.map((n) => map.get(n)), byName: map }
}
```
